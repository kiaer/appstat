...
Plotting the dataset gives an initial overview of the various enzyme responses and it is apparent that there is a difference in performance both between different enzymes as well as the individual enzymes at different concentrations. (See Figure \ref{fig:barplotinit})
...
To investiagte the relative effects of the different solutions and compare the spread of response values for each enzyme we generate boxplots. (See Figure \ref{fig:boxplotInit})  The initial descriptive interpretation confirms that there appears to be a significant correlation between the concentration of enzyme and the response and that further analysis should be conducted to investigate this further. The different enzymes also seem to perform differently and there is basis for analysing the performance difference in greater detail.
...
\section{Discussion}
When concidering the quality of the data, and whether there are systematic errors on the observations or not, the experiment setup has taken some of this into account. The main issue regarding this is that there may be different systematic errors on the different enzymes, as the enzymes has been tested on different days, and thus a "new" setup is used each time. When comparing tests containing 0 nM of enzyme and no detergent there is a variance between the different enzymes as seen in figure \ref{fig:Controlamples}. An analysis of the variance is also carried out, and results are seen in table \ref{tab:aov}.
...
The analysis shows that the variance between the controlsamples for the different enzymes is statistically significant, with a \code{p}-value of 0.0023. This should not be the case, as there are no enzymes involved in these observations, and therefore this is the strongest indication of systematic errors being present. This is, however, taken into account in the experiment setup by measuring a reference enzyme at each occation, making it possible to calibrate the measurements. These observations has not been available in the making of this report.

There is a possibility that there exist more overall systematic errors affecting all measurements equally. These may be difficult to find, but in this analysis it is not a big issue. As the main aim is to compare processes and effects of different components relative to eachother, this kind of systematic errors would not affect the outcome. 

\section{Conclusion}
Using a linear model on the data set, it is shown that the enzymes, enzyme concentration, detergent pressence and the interaction between enzymes and concentration is the statistical significant variables of the experiment.
  
The statistical analysis proves the different types of enzymes has a different influence on the response. From our analysis it is clearly seen that the Enzyme A provides the best reponse, while Enzyme D has the worst response. Every other enzyme (B,D,E) can be categorised with a nearly identical response, and no there is no significant difference between them and their influence on the response. It is also worth noting that the influence of concentration is higher than that of the type of enzyme. A "worse" enzyme will outperform the better one at higher concentration levels, and therefore considering using the enzymes with a lower response, but with a higher concentration can be considered. This could be investigated further if cost of the enzymes is introduced to the problem.
