...
?????? Description of the analysis that will be applied????
...
@
\section{Analysis}
<<boxcox, echo=FALSE, fig.height=5, out.width='.9\\linewidth', fig.cap="Boxcox Transformation of LM1. Based on this result a lambda value of 0.4 is chosen to transform the response observations.", fig.pos="H">>=
...
The diagnostic plots of the final model shows that performance is satisfactory. Normality is fulfilled, as seen in the Normal Q-Q plot, and variance is not varying with fitted values. Looking at leverage it also looks fairly safe to say that this linear model is indeed a good fit for the data. 
...
As a result of the analysis, the finished linear model based on the data set has the coefficients described in \ref{tab:sum}. 

<<summary, results='asis', echo=FALSE>>=
xtable(summary(lm4), caption = "Summary of the linear model", label = "tab:sum")
@

Looking at variance analysis (Tabel \ref{tab:anova}), it is clearly seen
...
@
The linear model LM4 is therefore the best linear model where both response and concentration is transformed used $x^{0.4}$. 
