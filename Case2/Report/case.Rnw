...
xtable(summary(camptable), caption = "Summary of the climatic variables (columns 1-5), the total number of flocks (column 6), and the number of Campylobacter positive flocks (column 7) ", label = "tab:sum")
@
...
From the data it is obvious that the climatic variables seem to cycle once per year and that the sunlight hours and temperature peaks in the summer and dip during the winter. The relative humidity has a cycle of the same period but reversed with the lowest relative humidity in the summer and highest in the winter. ((table XX)). 

<<plotcase, echo=FALSE, fig.pos="H",  fig.cap="The climatic variables through the year (plots 1-5) and the proportion of Campylobacter positive flocks (plot 6)">>=
par(mfrow=c(2,3))
