\section{Analysis}
<<>>
corvar<-data.frame(facit$pos/facit$total,facit[,3:7])
...
cortable[4,1]<-round(cor.test(corvar$facit.pos.facit.total,corvar$relHum)$estimate,2)
cortable[5,1]<-round(cor.test(corvar$facit.pos.facit.total,corvar$sunHours)$estimate,2)
