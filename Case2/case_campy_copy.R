...
lm1 <- lm((pos / total) ~ (aveTemp + maxTemp + relHum + sunHours + precip + week)^3, data=camp)
par(mfrow=c(2,2))
plot (lm1)
summary(lm1)
boxcox(lm1)
...
anova(lm3)
drop1(lm3, test = "F")
lm4 <- update(lm3, .~. -aveTemp:sunHours)
drop1(lm4, test="F")
lm5 <- step(lm4)
drop1(lm5, test="F")
lm6 <- update(lm5, .~. -maxTemp:sunHours)
drop1(lm6, test="F")
lm7 <- update(lm6, .~. -aveTemp:relHum:sunHours)
drop1(lm7, test="F")
...
drop1(lm11, test="F")
anova(lm11)
#Sunhours er mindre signifikant, men stadig signifikant. Beholde eller droppe?
